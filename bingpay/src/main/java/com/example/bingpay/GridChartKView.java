...
/**
  * 在232line 做了修改，要想要其它的MADJ图形效果，把注释去掉
  * Author:  Mr.bingley
  * Version:
  * Date:  2017/3/30
  */
...
// ////////////默认值////////////////
   ...
   private void drawLatitudes(Canvas canvas, int viewWidth, float latitudeSpacing ) {
     ...
     //  canvas.drawPath(path, paint);   // 控制最高最低，横坐标的横线。
   ...
   protected void drawWithFingerClick(Canvas canvas) {
       ...
       mPaint.setColor(getResources().getColor(R.color.colorPrimary));  // 控制十字线颜色
   ...
   private void drawAlphaXTextBox(String content, Canvas canvas) {
       ...
       mPaintBox.setColor(DEFAULT_TIP_BACKGROUND_COLOR);
       //mPaintBox.setAlpha(150);

       Paint mPaintBoxLine = new Paint();
       mPaintBoxLine.setColor(Color.BLUE);
       ...
       Paint borderPaint = new Paint();  // 这个控制下面的矩形
       borderPaint.setColor(DEFAULT_TIP_BACKGROUND_COLOR);
